...
class SingleOpAttnBlock(nn.Module):
    ...
    def forward(self, h, op_mask, attention_bias=None):
        """
        ...
        if attention_bias is not None:
            if attention_bias.shape != (sz_b, N, 3):
                raise ValueError("operation attention bias must be [B,N,3]")
            e = e + attention_bias.unsqueeze(-2).to(e)
...
class MultiHeadOpAttnBlock(nn.Module):
    ...
    def forward(self, h, op_mask, attention_bias=None):
        ...
        h_heads = [att(h, op_mask, attention_bias) for att in self.attentions]
...
class SingleMchAttnBlock(nn.Module):
    ...
    def forward(self, h, mch_mask, comp_val, attention_bias=None):
        """
        ...
        if attention_bias is not None:
            if attention_bias.shape != e.shape:
                raise ValueError("machine attention bias must be [B,M,M]")
            e = e + attention_bias.to(e)
...
class MultiHeadMchAttnBlock(nn.Module):
    ...
    def forward(self, h, mch_mask, comp_val, attention_bias=None):
        ...
        h_heads = [att(h, mch_mask, comp_val, attention_bias) for att in self.attentions]
